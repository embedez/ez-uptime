...
export const Notification = ({data, loading, className}:{className?: string, data: typeData, loading?: boolean}) => {
    ...
    return <div className={`${className ? `h-full flex flex-col justify-between gap-1` : className} ${loading ? 'animate-pulse' : ''}`}>
        ...
        <p className={`indent-4 break-words text-start line-clamp-4 group-focus-within:line-clamp-none whitespace-break-spaces ${loading ? 'bg-gray-600 opacity-80 min-h-[96px] w-full rounded-token animate-pulse' : ''}`}>{data.description || ""}</p>
        <div className={'w-full flex flex-row items-center justify-between gap-4'}>
            ...
            <div className={'flex flex-col flex-wrap text-sm text-justify max-w-fit'}>
